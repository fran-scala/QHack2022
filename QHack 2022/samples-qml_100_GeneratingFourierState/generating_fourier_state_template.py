...
def generating_fourier_state(n_qubits, m):
    ...
    def circuit(angles):
        ...
        # QHACK #
        bits = [int(i) for i in np.binary_repr(m, width=n_qubits)]
    ...
    def error(angles):
        ...
        # QHACK #
        comp_state = np.zeros(2**n_qubits)
        comp_state[m] = 1
        
        # The return error should be smaller when the state m is more likely to be obtained.
        return (comp_state[m]-probs[m])**2 #np.mean((comp_state-probs)**2)
        # QHACK #
...
if __name__ == "__main__":
    ...
    print(",".join([f"{p.real.round(5)},{p.imag.round(5)}" for p in
                    check_with_arbitrary_state()]))
