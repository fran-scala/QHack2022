...
def hamiltonian_coeffs_and_obs(graph):
    ...
    n = np.array((np.array([[1,0],[0,-1]])+1)/2, requires_grad=True)
    
    coeffs = [float(-1) for i in range(num_vertices)]
    obs = [qml.Hermitian(n, wires=i) for i in range(num_vertices)]
    for i in range(E.shape[0]-1):
        for j in range(i+1, E.shape[0]):
            if E[i][j]:
                       
                obs.append(qml.Hermitian(n, wires=i) @ qml.Hermitian(n, wires=j))
...
def variational_circuit(params, num_vertices):
    ...
    # for i in range(num_vertices):
    #     qml.RZ(params[num_vertices+i], wires=i)
...
def train_circuit(num_vertices, H):
    ...
    dev = qml.device("default.qubit", wires=num_vertices, shots=1)
    ...
    params = np.array(2*np.pi*np.random.rand(num_vertices), requires_grad=True)
    opt = qml.optimize.AdagradOptimizer(.3)
    epochs = 500
